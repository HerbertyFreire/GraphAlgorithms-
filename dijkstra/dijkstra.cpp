...
struct Path* dijkstra(struct Graph* graph, int s)
{
	struct Heap* heap;
	struct Path* path;
	...
	path->pre[s-1] = s;
	heap = newHeap(graph->e); // initialize heap
	insert(heap, s, path->dist[s-1]);
	...
	while (!isEmpty(heap)){
		...
		for (int i=0; i<graph->adj[u-1].size(); i++){
			v = graph->adj[u-1][i];
			if (path->dist[v-1] > path->dist[u-1] + graph->w[u-1][v-1]){
				path->dist[v-1] = path->dist[u-1] + graph->w[u-1][v-1];
				path->pre[v-1] = u;
...
void writeToFile(string filename, int start, int end, bool showSolution, struct Path* path, struct Graph* graph)
{
	...
	// Print distance from starting vetrex to all others veteces
		...
		for (int i=0; i<graph->v; i++){
			if (path->dist[i] == INT_MAX)	
				file << i+1 << ":inf ";
			else
				file << i+1 << ":" << path->dist[i] << ' ';
		}
	}
	file.close();
}
...
int main(int argc, char* argv[])
{
	...
	if (filename && start){
		...
		struct Path* path = dijkstra(graph, start);
		if (fileout)
